...
```php
$table->enum('role', ['admin', 'evaluator', 'auditor'])->default('evaluator');
```
...
Crear `app/Policies/AssessmentPolicy.php`:
- `viewAny`: admin ve todo, evaluator ve su empresa, auditor ve empresas asignadas.
- `view`: admin siempre, evaluator si misma empresa, auditor si empresa asignada.
- `create`: admin o evaluator.
- `update`: admin o creador.

Registrar en `AppServiceProvider` o via `AuthServiceProvider`.
